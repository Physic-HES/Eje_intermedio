...
# 0. Cargar y procesar la pieza STL
def load_and_process_mesh(filename, target_scale=1.0):
    mesh = trimesh.load(filename)
    # Centrar en el centro de masa
    mesh.apply_translation(-mesh.center_mass)
    
    # Alinear con los ejes principales de inercia
    ...
    mesh_scaled.apply_scale(target_scale / max_dim)
    
    return mesh_scaled, I_principal

# ... (Cargar y procesar la pieza STL)
mesh_body, I_vals = load_and_process_mesh('Ecrou_Papillon_M12.stl', target_scale=1.5)
I1, I2, I3 = I_vals
print(f"Momentos de inercia calculados: I1={I1:.2f}, I2={I2:.2f}, I3={I3:.2f}")

# Preguntar al usuario si desea generar el video
generar_video = input("¿Desea generar la animación y guardar el video? (s/n): ").lower() == 's'

# 1. Parámetros y Ecuaciones de Euler (Sistema del Cuerpo)
# ... (resto del código igual hasta la sección 4)
...
for i in range(3):
    e_i = R_final[:, i]
    ax3d_final.quiver(0, 0, 0, e_i[0], e_i[1], e_i[2], color=colors[i], lw=2, label=f'Eje {i+1}', arrow_length_ratio=0.15)
wf_final = w_fixed_t[-1]
ax3d_final.quiver(0, 0, 0, wf_final[0], wf_final[1], wf_final[2], color='black', lw=3, label=r'$\vec{\omega}$', arrow_length_ratio=0.15)
trace_data = R_t[:, :, 1]
...
ax3d_final.legend()
fig_3d.savefig('figura_3d_v5.png')
plt.close(fig_3d)
...
plt.legend()
plt.grid(True)
plt.savefig('figura_omega_cuerpo_v5.png')
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig('figura_omega_fijo_v5.png')
plt.close()

# 5. Animación Interactiva (Layout original 16:9 sin el gráfico de error)
if generar_video:
    print("Iniciando preparación de la animación...")
    ...
    def update(num, pbar):
        ...
        # Eliminar quivers anteriores para redibujar con puntas de flecha
        ...
        for i in range(3):
            e_i = R[:, i]
            q = ax3d.quiver(0, 0, 0, e_i[0], e_i[1], e_i[2], color=colors[i], lw=2, arrow_length_ratio=0.15)
            quiver_artists.append(q)
        
        q_w = ax3d.quiver(0, 0, 0, wf[0], wf[1], wf[2], color='black', lw=3, arrow_length_ratio=0.15)
        quiver_artists.append(q_w)
        ...
        pbar.update(1)
        return [trace3d, poly_anim] + lines_body + lines_fixed + quiver_artists

    # Configurar el progreso
    pbar = tqdm(total=len(t_eval), desc="Renderizando video")
    ...
    # Guardar como video
    print("Guardando video 'simulacion_euler_v5.mp4'...")
    try:
        writer = FFMpegWriter(fps=30, metadata=dict(artist='Me'), bitrate=1800)
        ani.save('simulacion_euler_v5.mp4', writer=writer)
    except Exception as e:
        print(f"Error guardando con FFMpegWriter: {e}")
        print("Intentando guardar con el escritor por defecto (puede requerir ffmpeg instalado)...")
        ani.save('simulacion_euler_v5.mp4', fps=30)

    pbar.close()
else:
    print("Generación de video omitida por el usuario.")
