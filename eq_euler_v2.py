...
def load_and_process_mesh(filename, target_scale=1.0):
    ...
    # Esto hace que el tensor de inercia sea diagonal
    transform = mesh.principal_inertia_transform
    mesh.apply_transform(transform)
    
    # Calcular momentos de inercia (asumiendo acero 7850 kg/m^3)
    # Unidades STL: mm -> kg/mm^3: 7850e-9
    mesh.density = 7850e-9
    I_matrix = mesh.moment_inertia
    I_principal = np.diag(I_matrix)
    
    # Simplificar para la animación (reducir carga computacional)
    try:
        mesh_simple = mesh.simplify_quadric_decimation(face_count=2000)
    except:
        mesh_simple = mesh
        
    # Escalar para visualización
    max_dim = np.ptp(mesh_simple.bounds, axis=0).max()
    mesh_simple.apply_scale(target_scale / max_dim)
    
    return mesh_simple, I_principal
...
ax3d_final.legend()
fig_3d.savefig('figura_3d_v2.png')
plt.close(fig_3d)
...
plt.legend()
plt.grid(True)
plt.savefig('figura_omega_cuerpo_v2.png')
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig('figura_omega_fijo_v2.png')
plt.close()
...
def update(num):
    ...
    return lines3d + [omega_line3d, trace3d, poly_anim] + lines_body + lines_fixed
